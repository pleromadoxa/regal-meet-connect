...
export const AudioOnlyGrid = ({
  ...
  const getParticipantInfo = (streamId: string, streamUserName: string, isLocal = false) => {
    if (isLocal) {
      return {
        name: userName,
    ...
    const participant = participants.find(p => p.user_id === streamId);
    return {
  ...
  const AudioParticipantCard = ({ 
    ...
    const participantInfo = getParticipantInfo(streamId, participantName, isLocal);
