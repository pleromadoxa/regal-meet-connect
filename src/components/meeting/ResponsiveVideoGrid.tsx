...
export const ResponsiveVideoGrid = ({
  ...
  const getParticipantInfo = (streamUserName: string, isLocal: boolean) => {
    if (isLocal) {
      return {
        name: userName,
    ...
    const p = participants.find((x) => x.user_name === streamUserName);
    return {
  ...
  const sheetParticipants = useMemo(
    () =>
      allStreams.map((s) => {
        const info = getParticipantInfo(s.userName, s.isLocal);
        return {
          id: s.id,
          name: info.name,
          isHost: info.isHost,
